...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  const hasGeminiKey = !!(env.GOOGLE_GENERATIVE_AI_API_KEY || env.GEMINI_API_KEY);
...
app.use('*', async (c, next) => {
  ...
  const sandbox = getSandbox(c.env.Sandbox, 'moltbot-v2026-final', options);
...
async function scheduled(
  ...
  const sandbox = getSandbox(env.Sandbox, 'moltbot-v2026-final', options);
